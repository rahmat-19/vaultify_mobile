...
class _VaultFormPageState extends ConsumerState<VaultFormPage> {
  ...
  @override
  void dispose() {
    title.dispose(); username.dispose(); secret.dispose(); website.dispose(); notes.dispose();
    super.dispose();
  }
  @override
  Widget build(BuildContext context) => SecureScreen(
    child: Scaffold(
      appBar: AppBar(title: Text(widget.item == null ? AppStrings.addItem : AppStrings.editItem)),
      body: SingleChildScrollView(
        padding: const EdgeInsets.all(20),
        child: Form(
          key: formKey,
          child: Column(children: <Widget>[
            AppTextField(controller: title, label: 'Judul',
              validator: (v) => v?.trim().isNotEmpty == true ? null : 'Judul wajib diisi.'),
            const SizedBox(height: 12),
            DropdownButtonFormField<VaultCategory>(
              initialValue: category,
              decoration: const InputDecoration(labelText: 'Kategori'),
              items: VaultCategory.values.map((value) =>
                DropdownMenuItem(value: value, child: Text(value.label))).toList(),
              onChanged: (value) => setState(() => category = value ?? category),
            ),
            const SizedBox(height: 12),
            AppTextField(controller: username, label: 'Username / identifier'),
            const SizedBox(height: 12),
            PasswordTextField(controller: secret, label: 'Kata sandi / rahasia',
              validator: (v) => v?.isNotEmpty == true ? null : 'Rahasia wajib diisi.'),
            const SizedBox(height: 12),
            AppTextField(controller: website, label: 'Website'),
            const SizedBox(height: 12),
            AppTextField(controller: notes, label: 'Catatan aman', maxLines: 4),
            const SizedBox(height: 20),
            PrimaryButton(label: AppStrings.save, loading: saving, onPressed: _save),
          ]),
        ),
      ),
    ),
  ...
  Future<void> _save() async {
    ...
    final ok = await ref.read(vaultControllerProvider.notifier).save(
      VaultItemInput(
        title: title.text.trim(), username: username.text.trim(),
        secret: secret.text, website: website.text.trim(),
        category: category, notes: notes.text,
      ),
      id: widget.item?.id,
    );
    if (!mounted) return;
